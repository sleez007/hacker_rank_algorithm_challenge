...
const diagonalDifference = (arr) => {
    const sqrRoot = Math.sqrt(arr.length);
    ...
    while(i <= arr.length){
        if(currPack.length >= sqrRoot){
            pack.push(currPack);
            sumPrimaryDiagonal += currPack[pack.length-1];
            sumSecondaryDiagonal +=  currPack[sqrRoot - pack.length]
            currPack = [];
        }
        currPack.push(arr[i]);
        ++i;
